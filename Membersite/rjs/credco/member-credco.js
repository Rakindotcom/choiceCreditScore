...
function sendNetPromoterScore(a, b) {
    var c = $(b).data("promoter-score");
    b = $(b).parents("#step1");
    "true" != $(b).data("npsClicked") && ($(b).data("npsClicked", "true"),
    $.ajax({
        dataType: "jsonp",
        url: "/Membersite/member/save_net_promoter_score.ajax",
        data: {
            netPromoterScore: c,
            scoreLocation: window.location.pathname
        },
        success: function(b) {
            localStorage.setItem("nps_saved", "true");
            $(a).data("testimonialId", b.testimonialId);
            $(a + " #step1").slideToggle();
            $(a + " #step2").slideToggle();
            $(a + " #step2").find("#labelContent").html(b.labelContent);
            0 <= b.labelContent.indexOf("phone") && $('\x3cinput id\x3d"phoneNumber" type\x3d"text" class\x3d"form-control" placeholder\x3d"Your Phone Number"/\x3e').insertAfter($(a + " #step2").find("#labelContent"))
        }
    }))
}
function nextStepNetPromoterScore(a) {
    console.log("nextStepNetPromoterScore called");
    a = $(element).parents(".modal.net-promo");
    $("step3", a).slideToggle();
    $("step4", a).slideToggle()
}
function sendNetPromoterText(a) {
    var b = $(a).data("testimonialId");
    $.ajax({
        dataType: "jsonp",
        url: "/Membersite/member/save_net_promoter_text.ajax",
        data: {
            testimonialId: b,
            netPromoterText: $(a + " textarea.form-control.step-2-feedback").val(),
            phoneNumber: $(a + " #step2 #phoneNumber").val()
        },
        success: function(b) {
            b.isNegativeTestimonial ? $(a + " #step3").addClass("feedback-negative") : b.isPositiveTestimonial ? 0 < $(a + " #step3 .feedback-positive").length ? $(a + " #step3").addClass("feedback-positive") : $(a + " #step3").addClass("feedback-neutral") : $(a + " #step3").addClass("feedback-neutral");
            $(a + " #step2").slideToggle();
            $(a + " #step3").slideToggle()
        }
    })
}
function sendNetPromoterNeedCSRContact(a, b) {
    a = $(a).data("testimonialId");
    (b = $(b).data("need-csr-contact")) && $.ajax({
        dataType: "jsonp",
        url: "/Membersite/member/request_csr_contact.ajax",
        data: {
            testimonialId: a,
            needSupportContact: b
        }
    })
}
;